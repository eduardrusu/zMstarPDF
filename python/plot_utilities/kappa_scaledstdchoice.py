...
f3 = np.genfromtxt(file3,usecols=[0,3,4],dtype='S200,f,f')
f1name = np.array([])
f1los = np.array([])
f1std = np.array([])
f2name = np.array([])
...
for i in range(len(matches)):
    out_ = np.array([matches[i],int(f1los[np.where(f1name == matches[i])][0]),int(f2los[np.where(f2name == matches[i])][0]),int(f3los[np.where(f3name == matches[i])][0]),np.around(float(f1std[np.where(f1name == matches[i])][0]),3),np.around(float(f2std[np.where(f2name == matches[i])][0]),3),np.around(float(f3std[np.where(f3name == matches[i])][0]),3)])
    out = np.c_[out,out_]

fout = "/Users/cerusu/Dropbox/Davis_work/code/WFI2033/kappasim/scaledstdchoice.dat"
np.savetxt(fout,out.T,fmt='%s %s %s %s %s %s %s')
