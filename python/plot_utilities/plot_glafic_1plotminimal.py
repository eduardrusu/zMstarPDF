...
def plot_points(prefix, title=None, show_src_in_implane=True, scale=0.03,
   ...
   for i in range(xi.size):
       #im = patches.Circle((xi[i],yi[i]),ri[i],color='c',alpha=0.8,label='t(%s): %6.2f' % (labi[i],tt[i]))
...
def make_model_plot(dtboxloc='tr'):
   """
   A top-level function to call plot_crit, plot_lensmod, and plot_points
   in order to make a nice plot.

   For now, assumes that all of the models will have prefixes of the form out_
   """

   prefix = "out"
   lensname = "PS1134-2103"
   ...
   #ymin,ymax = plt.ylim()
   plot_lensmod(prefix)
   #plot_points(prefix,lensname,xmin=xmin,xmax=xmax,ymin=ymin,ymax=ymax,dtboxloc=dtboxloc)
   plot_points(prefix,lensname,dtboxloc=dtboxloc)
