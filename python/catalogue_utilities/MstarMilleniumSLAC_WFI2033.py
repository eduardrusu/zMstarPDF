# The code computes stellar masses with LePhare for the galaxies in the Millennium Simulation, using a set of ugriz as well as ugrizJHK filters. For ugriz it does this only for the photometric redshift estimated in a previous run with photozMillenium_WFI2033.py, while for ugrizJHK it does it for the "true" catalogue redshift as well.
# Run on SLAC batch as python /scratch/cerusu/MstarMilleniumSLAC_WFI2033.py /scratch/cerusu/GGL_los_8_7_7_3_3_N_4096_ang_4_SA_galaxies_on_plane_27_to_63_ugrizJHK_WFI2033.images_forlepharetest.txt
...
if "ugrizJHK" in file:
    ...
    str = "ID \t u \t u_err \t g \t g_err \t r \t r_err \t i \t i_err \t z \t z_err \t J \t J_err \t H \t H_err \t Ks \t Ks_err \t context \t z-spec \t string"
    dataout = np.c_[data[id],data[u],data[u_err],data[g],data[g_err],data[r],data[r_err],data[i],data[i_err],data[z],data[z_err],data[photoz]]
    #np.savetxt(fileinphotoz,dataout,header=str,fmt='%d \t %.2f \t %.2f \t %.2f \t %.2f \t %.2f \t %.2f \t %.2f \t %.2f \t %.2f \t %.2f \t -99.00 \t -99.00 \t -99.00 \t -99.00 \t -99.00 \t -99.00 \t 255 \t %.2f')
    np.savetxt(fileinphotoz,dataout,fmt='%d \t %.2f \t %.2f \t %.2f \t %.2f \t %.2f \t %.2f \t %.2f \t %.2f \t %.2f \t %.2f \t -99.00 \t -99.00 \t -99.00 \t -99.00 \t -99.00 \t -99.00 \t 255 \t %.2f')
    os.system("/scratch/cerusu/runlephare_phys_para_millenium.sh %s" % fileinphotoz)
    os.system("rm %s" % fileinphotoz[16:])
    mstar_photoz = np.loadtxt(fileinphotoz + ".MAG_BC03_I09.lephareout",usecols=[33,35],unpack=True)
    mstar_photoz[0][mstar_photoz[1] > 0] = mstar_photoz[1][mstar_photoz[1] > 0]
    mstar_photoz[0][mstar_photoz[0] < 0] = 9
    #str = "ID \t mstar_photoz"
    #np.savetxt(fileout,np.c_[data[id],mstar_photoz[0]],header=str,fmt='%d \t %.3f')
    np.savetxt(fileout,np.c_[data[id],mstar_photoz[0]],fmt='%d \t %.3f')
    os.system("rm %s" % fileinphotoz[16:] + ".MAG_BC03_I09.lephareout")
