...
# Run as python finalprepMilleniumNAOJ.py GGL_los_8_0_0_0_0_N_4096_ang_4_SA_galaxies_on_plane_27_to_63_ugriz.images.txt
...
#fileextract = "GGL_los_8_0_0_0_0_N_4096_ang_4_SA_galaxies_on_plane_27_to_63_ugrizJHK_WFI2033.images.txt"
#fileforlephare = "GGL_los_8_0_0_0_0_N_4096_ang_4_SA_galaxies_on_plane_27_to_63_ugrizJHK_WFI2033.images_forlephare.txt"
#filemstar = "GGL_los_8_0_0_0_0_N_4096_ang_4_SA_galaxies_on_plane_27_to_63_ugrizJHK_WFI2033.images_forlephare_mstar.txt"
fileforlephare = fileextract[:-4] + "_forlephare.txt"
filemstar = fileextract[:-4] + "_forlephare_mstar.txt"
fileout = fileextract[:-4] + "_forNAOJ.txt"
...
if "ugrizJHK" in fileextract:
    ...
    dataextract = np.loadtxt(fileextract,usecols=[id,zspec,posx,posy,mhalo,mstar,iorig,i_o],unpack=True)
    dataforlephare = np.loadtxt(fileforlephare,usecols=[photoz],unpack=True)
    datamstar = np.loadtxt(filemstar,usecols=[mstar_photoz],unpack=True)
...
if "ugrizJHK" in fileextract:
    ...
    d = d0_ + da_ * (a - 1)
    g = g0_ + ga_ * (a - 1)
    datahalo[:,1][dataforlephare <= 1] = logM1a + b * (datamstar[dataforlephare <= 1] - logMs0a) + ((10 ** datamstar[dataforlephare <= 1]/notlogMs0a)**d)/(1+(10 ** datamstar[dataforlephare <= 1]/notlogMs0a)**(-g)) - 1/2
    del logM1a
    del logMs0a
    del notlogMs0a
    del b
...
if "ugrizJHK" in fileextract:
    ...
    dataout = np.c_[dataextract[id],dataextract[zspec],dataextract[posx],dataextract[posy],dataextract[mhalo],dataextract[mstar],dataextract[iorig],dataextract[i_o],dataforlephare,datamstar,datahalo[:,1]]
    head = "GalID \t z_spec \t pos_0 \t pos_1 \t M_Halo \t M_Stellar \t mag_SDSS_iorig \t mag_SDSS_i \t photoz \t mstar_photoz \t mhalo_photoz"
    np.savetxt(fileout,dataout,header=head,fmt='%d \t %.3f \t %.7f \t %.7f \t %.3e \t %.3e \t %.2f \t %.2f \t %.2f \t %.3f \t %.3f')
