# testing assuming all specz and a single grid (not multiple overlaping grids)
# run with the following arguments: lens name, field name, limiting mag, outer mask radius, type, inner mask radius, zinf, zsup (in case I remove redshift slices); e.g.: python /lfs08/rusucs/code/kappamed_insertstarsnobetanomass_testonly.py J1206 GGL_los_8_0_0_N_4096_ang_4_rays_to_plane_34_f 23 45 measured 5 -1.0 -1.0
...
def weightedcounts(cat,spacing,lim1D,cells_on_a_side,L_field,L_pix,cells,kappagamma,pln,bands):
    ...
    for i in range(spacing):
        for j in range(spacing):
            ...
            p_sum = pd.DataFrame({'cell':cat_msk[:,index_index].astype(int),'sum':1.0})
            p_zweight = pd.DataFrame({'cell':cat_msk[:,index_index].astype(int),'zweight':1.0 * (z_s * cat_msk[:,index_z]) - cat_msk[:,index_z]**2})
            ...
            w_sum_2X = p_sum.groupby(['cell']).sum().values[:,0]
...
rootwghtratios = "/Users/cerusu/Desktop/test/MSweights/"
...
rootgals = "/Users/cerusu/Desktop/test/MSgals/"
