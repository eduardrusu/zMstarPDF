...
if handpicked == '':
    ...
    lst45 = [x for x in os.listdir(rootin) if ('samples' in x) and ('75nolim.lst' in x) and ('PG1115_weightedcountshist_45arcsec_5inner_23_meds_bpz%s' %handpicked in x) ] #  # CHOOSE WHAT YOU WANT HERE
    lst120 = [x for x in os.listdir(rootin) if ('samples' in x) and ('75nolim.lst' in x) and ('PG1115_weightedcountshist_120arcsec_5inner_23_meds_bpz%s' %handpicked in x) ]
...
for i in range(len(lst45)):
    ...
    if type(bpz_detir_irac45) == int: bpz_detir_irac45 = np.loadtxt('%s%s' %(rootin,lst45[i]), unpack=True)
    ...
    if type(bpz_detir_irac45) != int: bpz_detir_irac45 = np.c_[bpz_detir_irac45,np.loadtxt('%s%s' %(rootin,lst45[i]), unpack=True)]
...
for i in range(len(lst120)):
    ...
    if type(bpz_detir_irac120) == int: bpz_detir_irac120 = np.loadtxt('%s%s' %(rootin,lst120[i]), unpack=True)
    ...
    if type(bpz_detir_irac120) != int: bpz_detir_irac120 = np.c_[bpz_detir_irac120,np.loadtxt('%s%s' %(rootin,lst120[i]), unpack=True)]
