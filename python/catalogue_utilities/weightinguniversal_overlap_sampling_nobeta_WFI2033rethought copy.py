# This code makes use of the *galphotmstar.cat files and the lens photometric+Mstar+photoz catalogue; it computes weighted ratios (lens/field) with proper masking, for various radii, limiting mag, number of samples and classification scheme.
# run from /Volumes/perseus_1/CFHTLens_galphotmstar/ (where the scripts are) as: python /Users/perseus/Dropbox/Davis_work/code/GOODCODE/weightinguniversal_overlap_sampling_nobeta_WFI2033.py WFI2033 /Volumes/perseus_1/CFHTLens_galphotmstar/W1m0m0_24galphotmstar.cat /Users/perseus/Desktop/CFHTLenSmasks/W1m0m0_izrgu_finalmask_mosaic.fits 45 IRAC deti meds
# the scripts from running multiple processes are in /Volumes/perseus/Desktop/CFHTLens_galphotmstar/
...
if lensID == "WFI2033":
    ...
    inner = 5 # inner mask cut radius in arcsec
...
#rootlensmask = "/Users/perseus/Dropbox/Davis_work/code/CODETOCHECK"
rootlenscat = '/Users/perseus/Dropbox/Davis_work/code/%s' % lensID # where the lens catalogue and its masks are
...
msk_lens = fits.open('%s/msk%s_asecrad%s_no%sarcsec.fits' % (rootlenscat,lensID,radius,inner))
...
coord_lensinit = SkyCoord(ra=lensbpz[RA_lens]*u.degree, dec=lensbpz[DEC_lens]*u.degree, frame='fk5')
...
lensbpz = np.c_['0',lensbpz,sep_lens.reshape((1, sep_lens.shape[0]))]
lensbpz = np.delete(lensbpz,np.where(msk_lens[0].data[lensbpz[y_lens].astype(int),lensbpz[x_lens].astype(int)] != 0),axis=1) # I tested that this is the correct order of x and y. x and y in lensbpz are the actual coordinates in the natural reading of the .fits file (not the reading of python, which inverts axes)
#print msk_lens[0].data[400,485] # testing
lensbpz = np.delete(lensbpz,np.where(lensbpz[classify] < 0),axis=1) # removes all stars from the catalogue
#for i in range(np.shape(lensbpz)[1]): # used for testing
    #print lensbpz[:,i][x_lens],lensbpz[:,i][y_lens]
lensbpz[x_lens] = lensbpz[x_lens] - (pixnr - (2/pixlens.value) * radius)/2   # rescale the pixel coordinates in the original catalogue as appropriate for the relevant aperture radius; since the original is fractional, I am not using ((pixnr - (2/pixlens.value) * radius)/2).astype(int)
lensbpz[y_lens] = lensbpz[y_lens] - (pixnr - (2/pixlens.value) * radius)/2

coord_lensinit = SkyCoord(ra=lenseazy[RA_lens]*u.degree, dec=lenseazy[DEC_lens]*u.degree, frame='fk5')
sep_lens = coord_lensinit.separation(center_lens).arcsec
sep_lens[sep_lens < 10] = 10 # limiting the minimum distance to the lens to 10 arcsec

lenseazy = np.c_['0',lenseazy,sep_lens.reshape((1, sep_lens.shape[0]))]
lenseazy = np.delete(lenseazy,np.where(msk_lens[0].data[lenseazy[y_lens].astype(int),lenseazy[x_lens].astype(int)] != 0),axis=1)
lenseazy = np.delete(lenseazy,np.where(lenseazy[classify] < 0),axis=1) # removes all stars from the catalogue
lenseazy[x_lens] = lenseazy[x_lens] - (pixnr - (2/pixlens.value) * radius)/2   # rescale the pixel coordinates in the original catalogue as appropriate for the relevant aperture radius
lenseazy[y_lens] = lenseazy[y_lens] - (pixnr - (2/pixlens.value) * radius)/2
...
mskfrac = msk_lens[0].data[(pixnr - (2/pixlens.value) * radius)/2 : pixnr - (pixnr - (2/pixlens.value) * radius)/2,(pixnr - (2/pixlens.value) * radius)/2 : pixnr - (pixnr - (2/pixlens.value) * radius)/2]  # will be used later in computing the mask cover fraction; since it accepts fractional arguments, no need to change to int
...
print count
fcount = open('%s_%s_%s_%s_%s_%s_%sarcsec_count.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner),'w')
fcount.write(count)
fcount.close()

os.system('rm %s_50_%s_%s_%s_%s_%s_%sarcsec_0.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner))
os.system('rm %s_75_%s_%s_%s_%s_%s_%sarcsec_0.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner))
os.system('rm %s_50_%s_%s_%s_%s_%s_%sarcsec_1.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner))
os.system('rm %s_75_%s_%s_%s_%s_%s_%sarcsec_1.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner))
os.system('rm %s_50_%s_%s_%s_%s_%s_%sarcsec_2.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner))
os.system('rm %s_75_%s_%s_%s_%s_%s_%sarcsec_2.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner))
os.system('rm %s_50_%s_%s_%s_%s_%s_%sarcsec_3.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner))
os.system('rm %s_75_%s_%s_%s_%s_%s_%sarcsec_3.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner))
os.system('rm %s_50_%s_%s_%s_%s_%s_%sarcsec_4.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner))
os.system('rm %s_75_%s_%s_%s_%s_%s_%sarcsec_4.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner))
os.system('rm %s_50_%s_%s_%s_%s_%s_%sarcsec_5.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner))
os.system('rm %s_75_%s_%s_%s_%s_%s_%sarcsec_5.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner))
os.system('rm %s_50_%s_%s_%s_%s_%s_%sarcsec_6.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner))
os.system('rm %s_75_%s_%s_%s_%s_%s_%sarcsec_6.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner))
os.system('rm %s_50_%s_%s_%s_%s_%s_%sarcsec_7.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner))
os.system('rm %s_75_%s_%s_%s_%s_%s_%sarcsec_7.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner))
os.system('rm %s_50_%s_%s_%s_%s_%s_%sarcsec_8.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner))
os.system('rm %s_75_%s_%s_%s_%s_%s_%sarcsec_8.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner))
os.system('rm %s_50_%s_%s_%s_%s_%s_%sarcsec_9.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner))
os.system('rm %s_75_%s_%s_%s_%s_%s_%sarcsec_9.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner))

file50_0 = open('%s_50_%s_%s_%s_%s_%s_%sarcsec_0.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner),'a')
file75_0 = open('%s_75_%s_%s_%s_%s_%s_%sarcsec_0.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner),'a')
file50_1 = open('%s_50_%s_%s_%s_%s_%s_%sarcsec_1.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner),'a')
file75_1 = open('%s_75_%s_%s_%s_%s_%s_%sarcsec_1.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner),'a')
file50_2 = open('%s_50_%s_%s_%s_%s_%s_%sarcsec_2.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner),'a')
file75_2 = open('%s_75_%s_%s_%s_%s_%s_%sarcsec_2.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner),'a')
file50_3 = open('%s_50_%s_%s_%s_%s_%s_%sarcsec_3.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner),'a')
file75_3 = open('%s_75_%s_%s_%s_%s_%s_%sarcsec_3.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner),'a')
file50_4 = open('%s_50_%s_%s_%s_%s_%s_%sarcsec_4.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner),'a')
file75_4 = open('%s_75_%s_%s_%s_%s_%s_%sarcsec_4.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner),'a')
file50_5 = open('%s_50_%s_%s_%s_%s_%s_%sarcsec_5.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner),'a')
file75_5 = open('%s_75_%s_%s_%s_%s_%s_%sarcsec_5.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner),'a')
file50_6 = open('%s_50_%s_%s_%s_%s_%s_%sarcsec_6.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner),'a')
file75_6 = open('%s_75_%s_%s_%s_%s_%s_%sarcsec_6.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner),'a')
file50_7 = open('%s_50_%s_%s_%s_%s_%s_%sarcsec_7.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner),'a')
file75_7 = open('%s_75_%s_%s_%s_%s_%s_%sarcsec_7.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner),'a')
file50_8 = open('%s_50_%s_%s_%s_%s_%s_%sarcsec_8.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner),'a')
file75_8 = open('%s_75_%s_%s_%s_%s_%s_%sarcsec_8.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner),'a')
file50_9 = open('%s_50_%s_%s_%s_%s_%s_%sarcsec_9.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner),'a')
file75_9 = open('%s_75_%s_%s_%s_%s_%s_%sarcsec_9.lst' % (fieldID[0:-4],radius,lensID,det,irac,type,inner),'a')
