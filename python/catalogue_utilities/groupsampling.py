...
if lens == 'PG1115': zgroup = 0.3097 # PG1115
limmag = 22.5 # WFI2033
if lens == 'PG1115': limmag = 22.5 # PG1115, slightly fainter than faintest in Momcheva
...
print "gals: ",all
#specs120 = len(data[:,id][(sep <= 120) & (data[:,i] <= limmag) & (data[:,spec] > 0)])
if lens == 'PG1115':
...
print 'members inside 120\":',len(observed120_membersID) + 1
...
if mode == "poisson":
    def expected_members_noprior():
        while True:
            ...
            if lens == 'PG1115': x = np.random.poisson(1.0 * all * (len(observed120_membersID)+1)/specs120, 1) #PG1115
            break
        return x

pdz = np.array([])
if mode == "mcmc": theorysamples = 1000
if mode == "poisson": theorysamples = 10000
for i in range(theorysamples):
    if i % 10 == 0: print i,'/',theorysamples
    #print i
    expected = expected_members()
    if mode == "mcmc": pdz = np.append(pdz,expected[1] * expected[0])
...
for i in range(theorysamples):
    ...
    if mode == "mcmc": pdznoprior = np.append(pdznoprior,expected[1] * expected[0])
...
for i in range(samples):
    expected = expected_members()
    if mode == "mcmc":
        missing120_membersID = np.random.choice(a=pool, size=int(expected[1] * expected[0] - len(observed120_membersID)), replace=False)
        if lens == 'PG1115': missing120_membersID = np.random.choice(a=pool, size=int(expected[1] * expected[0] - len(observed120_membersID) - 1), replace=False)
    if mode == "poisson":
        missing120_membersID = np.random.choice(a=pool, size=int(expected[0] - len(observed120_membersID)), replace=False)
        if lens == 'PG1115': missing120_membersID = np.random.choice(a=pool, size=int(expected[0] - len(observed120_membersID) - 1), replace=False)
...
if lens == 'PG1115':
    ...
    plt.hist(pdzselect - len(observed120_membersID) - 1,normed=True,alpha = 0.5)
