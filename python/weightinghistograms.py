...
# usage: use one of the following arguments: orig or samp, followed by number of bins, followed by radius (45,60,90 or 120) and by maglimit
...
print("Arguments: \n Original values or samples drawn from P(z) and P(Mstar): %s \n Number of bins: %s \n Radius of each cell: %s \n Limiting i-band magnitude: %s" % (str(sys.argv[1]), str(sys.argv[2]), str(sys.argv[3]), str(sys.argv[4])))
...
with open('fieldsforhist50try_%s_size%s_i%s.lst' % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4]))) as f:
    listfields = f.readlines()

with open('fieldshistW1_50_%s_size%s_i%s.lst' % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])), 'w') as outfile:
...
with open('fieldshistW2_50_%s_size%s_i%s.lst' % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])), 'w') as outfile:
    for i in range(len(listfields)):
        if "W2" in [x[0:len(listfields[0])-1] for x in listfields][i]:
            with open([x[0:len(listfields[0])-1] for x in listfields][i]) as infile:
                outfile.write(infile.read())

with open('fieldshistW3_50_%s_size%s_i%s.lst' % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])), 'w') as outfile:
    for i in range(len(listfields)):
        if "W3" in [x[0:len(listfields[0])-1] for x in listfields][i]:
            with open([x[0:len(listfields[0])-1] for x in listfields][i]) as infile:
                outfile.write(infile.read())

with open('fieldshistW4_50_%s_size%s_i%s.lst' % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])), 'w') as outfile:
    for i in range(len(listfields)):
        if "W4" in [x[0:len(listfields[0])-1] for x in listfields][i]:
            with open([x[0:len(listfields[0])-1] for x in listfields][i]) as infile:
                outfile.write(infile.read())

with open('fieldsforhist75try_%s_size%s_i%s.lst' % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4]))) as f:
    listfields = f.readlines()

with open('fieldshistW1_75_%s_size%s_i%s.lst' % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])), 'w') as outfile:
    for i in range(len(listfields)):
        if "W1" in [x[0:len(listfields[0])-1] for x in listfields][i]:
            with open([x[0:len(listfields[0])-1] for x in listfields][i]) as infile:
                outfile.write(infile.read())

with open('fieldshistW2_75_%s_size%s_i%s.lst' % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])), 'w') as outfile:
    for i in range(len(listfields)):
        if "W2" in [x[0:len(listfields[0])-1] for x in listfields][i]:
            with open([x[0:len(listfields[0])-1] for x in listfields][i]) as infile:
                outfile.write(infile.read())

with open('fieldshistW3_75_%s_size%s_i%s.lst' % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])), 'w') as outfile:
    for i in range(len(listfields)):
        if "W3" in [x[0:len(listfields[0])-1] for x in listfields][i]:
            with open([x[0:len(listfields[0])-1] for x in listfields][i]) as infile:
                outfile.write(infile.read())

with open('fieldshistW4_75_%s_size%s_i%s.lst' % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])), 'w') as outfile:
    for i in range(len(listfields)):
        if "W4" in [x[0:len(listfields[0])-1] for x in listfields][i]:
            with open([x[0:len(listfields[0])-1] for x in listfields][i]) as infile:
                outfile.write(infile.read())
...
cols=1
q_W1_50read = np.loadtxt('fieldshistW1_50_%s_size%s_i%s.lst' % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])), usecols=[cols], unpack=True)
q_W1_50 = q_W1_50read[q_W1_50read < 10]
q_W2_50read = np.loadtxt('fieldshistW2_50_%s_size%s_i%s.lst' % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])), usecols=[cols], unpack=True)
q_W2_50 = q_W2_50read[q_W2_50read < 10]
q_W3_50read = np.loadtxt('fieldshistW3_50_%s_size%s_i%s.lst' % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])), usecols=[cols], unpack=True)
q_W3_50 = q_W3_50read[q_W3_50read < 10]
q_W4_50read = np.loadtxt('fieldshistW4_50_%s_size%s_i%s.lst' % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])), usecols=[cols], unpack=True)
q_W4_50 = q_W4_50read[q_W4_50read < 10]
q_W1_75read = np.loadtxt('fieldshistW1_75_%s_size%s_i%s.lst' % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])), usecols=[cols], unpack=True)
q_W1_75 = q_W1_75read[q_W1_75read < 10]
q_W2_75read = np.loadtxt('fieldshistW2_75_%s_size%s_i%s.lst' % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])), usecols=[cols], unpack=True)
q_W2_75 = q_W2_75read[q_W2_75read < 10]
q_W3_75read = np.loadtxt('fieldshistW3_75_%s_size%s_i%s.lst' % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])), usecols=[cols], unpack=True)
q_W3_75 = q_W3_75read[q_W3_75read < 10]
q_W4_75read = np.loadtxt('fieldshistW4_75_%s_size%s_i%s.lst' % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])), usecols=[cols], unpack=True)
...
plt.savefig('HE0435overdensities_%s_size%s_i%s.png' % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])), dpi=1000)

#plt.show()

os.system("rm fieldshistW1_50_%s_size%s_i%s.lst" % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])))
os.system("rm fieldshistW2_50_%s_size%s_i%s.lst" % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])))
os.system("rm fieldshistW3_50_%s_size%s_i%s.lst" % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])))
os.system("rm fieldshistW4_50_%s_size%s_i%s.lst" % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])))
os.system("rm fieldshistW1_75_%s_size%s_i%s.lst" % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])))
os.system("rm fieldshistW2_75_%s_size%s_i%s.lst" % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])))
os.system("rm fieldshistW3_75_%s_size%s_i%s.lst" % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])))
os.system("rm fieldshistW4_75_%s_size%s_i%s.lst" % (str(sys.argv[1]),str(sys.argv[3]),str(sys.argv[4])))
