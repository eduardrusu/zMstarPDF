...
# run as: python weightingsims_correctsamponly.py /Volumes/G-RAIDStudio/simulations/lensing_simulations/Guo_galaxies/GGL_los_8_7_6_0_0_N_4096_ang_4_Guo_galaxies_on_plane_27_to_63_pdzmstar_noJHKs.cat samplesize# radius maglimit
...
z_s_HE1104 = 2.32
z_s_RX1131 = 0.66
cosmdist=distances.Distance()
cosmdist.reset # to match Lephare cosmology
                ...
                if len(pdz_tab[pdz_tab!=0])!=0:
                    ...
                    for m in range(70):
                        if pdz_tab[m]!=0:
                            #print Mstar_med_zb
                            #print zgrid[m]
                            massmed_tab[m]=np.log10((10**Mstar_med_zb)*(cosmdist.luminosity_distance(zgrid[m])**2)/(cosmdist.luminosity_distance(Z_B)**2))
                            #print massmed_tab[m]
                            if massmed_tab[m]<0:
                                massmed_tab[m]=np.log10((10**Mstar_best_zb)*(cosmdist.luminosity_distance(zgrid[m])**2)/(cosmdist.luminosity_distance(Z_B)**2))
                            if massmed_tab[m]<0:# or (massmed_tab[m]<6) or (massmed_tab[m]>12):
                                massmed_tab[m]=9
                            #if (((massmed_tab[m]-Mstar_med_zb)>2) or ((massmed_tab[m]-Mstar_med_zb)<-2)):
                                #print Mstar_med_zb,massmed_tab[m],float(gal.split()[100 + (m+1) * 4 - 3]),catmass,"EXCEPT VAL!"
                                #massmed_tab[m]=Mstar_med_zb
                            #print Mstar_med_zb, (cosmdist.luminosity_distance(zgrid[m])**2)/(cosmdist.luminosity_distance(Z_B)**2), massmed_tab[m]
                            massinf_tab[m]=massmed_tab[m]-(Mstar_med_zb-Mstar_inf_zb)
                            masssup_tab[m]=massmed_tab[m]+(Mstar_sup_zb-Mstar_med_zb)
                            if massinf_tab[m]<0:
                            ...
                            #print Mstar_med_zb,massmed_tab[m],float(gal.split()[100 + (m+1) * 4 - 3])
                    ...
                    for l in range(int(str(sys.argv[2]))):
                        if sample_massmed_tab[l]==0:     #I SHOULD NOT HAVE TO DO THIS, THERE IS A BUG; actually I tested and this happens because of missing stellar mass for z=3.5 (the edge)
                            ...
                            if iter==0:
                                print "Exception!", ID
                            iter=iter+1
                        #print sample_massmed_tab[l], sample_masssup_tab[l]
...
#if str(sys.argv[8]) == "tab":
h = open('%s_B1608_samp_size%s_i%s.lst' % (str(sys.argv[1])[0:len(str(sys.argv[1]))-4],str(sys.argv[3]),str(sys.argv[4])),'w')
for i in range(cells_on_a_side[0]):
    for j in range(cells_on_a_side[1]):
