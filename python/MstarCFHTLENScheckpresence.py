...
# run from the /code folder as: python MstarCFHTLENScheckpresence.py /Users/perseus/Desktop/CFHTcatalogues/W4m0m0.cat /Users/perseus/Desktop/CFHTcatalogues/W4m0m0_mstar.cat nr
...
with open(str(sys.argv[1])) as search_for:
    for gal_for in search_for:
        ord=ord+1
        if gal_for!="\n":
            if gal_for.split()[0]!="#id" and ((float(gal_for.split()[79]) <= 24 and float(gal_for.split()[79]) > 0) or (float(gal_for.split()[84]) <= 24 and float(gal_for.split()[84]) > 0)):
