...
# Initialize all field weight measurements as blank matrices
        ...
        field_gal = np.zeros((cells_on_a_side,cells_on_a_side,int(str(sys.argv[3]))))
        field_oneoverr = np.zeros((cells_on_a_side,cells_on_a_side,int(str(sys.argv[3]))))
        field_zweight = np.zeros((cells_on_a_side,cells_on_a_side,int(str(sys.argv[3]))))
        field_mass = np.zeros((cells_on_a_side,cells_on_a_side,int(str(sys.argv[3]))))
        field_mass2 = np.zeros((cells_on_a_side,cells_on_a_side,int(str(sys.argv[3]))))
        field_mass3 = np.zeros((cells_on_a_side,cells_on_a_side,int(str(sys.argv[3]))))
        field_zoverr = np.zeros((cells_on_a_side,cells_on_a_side,int(str(sys.argv[3]))))
        field_massoverr = np.zeros((cells_on_a_side,cells_on_a_side,int(str(sys.argv[3]))))
        field_mass2overr = np.zeros((cells_on_a_side,cells_on_a_side,int(str(sys.argv[3]))))
        field_mass3overr = np.zeros((cells_on_a_side,cells_on_a_side,int(str(sys.argv[3]))))
        field_zmassoverr = np.zeros((cells_on_a_side,cells_on_a_side,int(str(sys.argv[3]))))
        field_zmass2overr = np.zeros((cells_on_a_side,cells_on_a_side,int(str(sys.argv[3]))))
...
# Use objects from the field catalogue that pass the redshift, separation, and mask tests
        ...
        for i in range(len(catfield)): #range(len(catfield)):   #range(1000):    JUST TO CHECK QUICKLY
            if i in [1000, 5000, 10000, 20000, 30000, 40000, 50000, 60000, 70000, 80000, 90000, 100000, 110000, 120000, 130000, 140000, 150000, 160000, 170000, 180000, 190000, 200000]:
                print i, "objects..."
            if (catfield['Z_B'][i] <= z_s0435) and (catfield['MASK'][i] == 0) and (catfield['star_flag'][i] == 0) and ((catfield['MAG_i'][i] <= 23 and catfield['MAG_i'][i] > 0) or (catfield['MAG_y'][i] <= 23 and catfield['MAG_y'][i] > 0)): # detection in either i or y bands
        ...
        #print coordfieldwcs
                ...
                if (maskedcell[x][y] >= 0.50):
                    if (sep >= 4) and (msk0435[0].data[(cellpix_y * pixCFHT / pixlens).value][(cellpix_x * pixCFHT / pixlens).value] == 0): # ignore objects too close to the center and masked in the lens catalogue
                        samplesup=np.zeros(int(str(sys.argv[3])))
                        sampleinf=np.zeros(int(str(sys.argv[3])))
                        fracz=1-1.0*(catfield['Z_B_MAX'][i]-catfield['Z_B'][i])/(catfield['Z_B_MAX'][i]-catfield['Z_B_MIN'][i])
                        fracmass=1-(1.0*(10**catfield['LP_log10_SM_SUP'][i]-10**catfield['LP_log10_SM_MED'][i])/(10**catfield['LP_log10_SM_SUP'][i]-10**catfield['LP_log10_SM_INF'][i]))
                        #print fracmass
                        if (fracz > 0) and (fracz < 1):
                            samplesup=catfield['Z_B'][i]+abs(np.random.normal(0, catfield['Z_B_MAX'][i]-catfield['Z_B'][i], int(str(sys.argv[3]))))
                            sampleinf=catfield['Z_B'][i]-abs(np.random.normal(0, catfield['Z_B'][i]-catfield['Z_B_MIN'][i], int(str(sys.argv[3]))))
                            # no negative redshifts
                            while len(sampleinf[sampleinf<0]) > 0:
                                sampleinf[sampleinf<0]=catfield['Z_B'][i]-abs(np.random.normal(0, catfield['Z_B'][i]-catfield['Z_B_MIN'][i], len(sampleinf[sampleinf<0])))
                            rand=np.random.random(int(str(sys.argv[3])))
                            samplez=sampleinf
                            samplez[np.where(rand>fracz)]=samplesup[np.where(rand>fracz)]
                            #samplez[samplez<0]=0
                        if (fracz <= 0) or (fracz >= 1):
                            samplez=np.random.normal(catfield['Z_B_MIN'][i]+(catfield['Z_B_MAX'][i]-catfield['Z_B_MIN'][i])/2, (catfield['Z_B_MAX'][i]-catfield['Z_B_MIN'][i])/2, int(str(sys.argv[3])))
                            #samplez[samplez<0]=0
                        # for mass, assume gaussian distribution in log, not normal space, so I don't get things like negative mass
                        if (fracmass > 0) and (fracmass < 1):
                            samplesup=10**(catfield['LP_log10_SM_MED'][i]+abs(np.random.normal(0, catfield['LP_log10_SM_SUP'][i]-catfield['LP_log10_SM_MED'][i], int(str(sys.argv[3])))))
                            sampleinf=10**(catfield['LP_log10_SM_MED'][i]-abs(np.random.normal(0, catfield['LP_log10_SM_MED'][i]-catfield['LP_log10_SM_INF'][i], int(str(sys.argv[3])))))
                            rand=np.random.random(int(str(sys.argv[3])))
                            samplemass=sampleinf
                            samplemass[np.where(rand>fracmass)]=samplesup[np.where(rand>fracmass)]
                        if (fracmass <= 0) or (fracmass >= 1):
                            samplemass=10**np.random.normal(catfield['LP_log10_SM_INF'][i]+(catfield['LP_log10_SM_SUP'][i]-catfield['LP_log10_SM_INF'][i])/2, (catfield['LP_log10_SM_SUP'][i]-catfield['LP_log10_SM_INF'][i])/2, int(str(sys.argv[3])))
                        ...
                        # ignore objects with z>z_source for all weights when drawing from the z PDF
                        field_gal[x][y][samplez<z_s0435] = field_gal[x][y][samplez<z_s0435] + 1
                        field_zweight[x][y][samplez<z_s0435] = field_zweight[x][y][samplez<z_s0435] + (z_s0435 * samplez[samplez<z_s0435]) - (samplez[samplez<z_s0435]**2)
                        field_mass[x][y][samplez<z_s0435] = field_mass[x][y][samplez<z_s0435] + samplemass[samplez<z_s0435]
                        field_mass2[x][y][samplez<z_s0435] = field_mass2[x][y][samplez<z_s0435] + samplemass[samplez<z_s0435]**2
                        field_mass3[x][y][samplez<z_s0435] = field_mass3[x][y][samplez<z_s0435] + samplemass[samplez<z_s0435]**3
                #print (catfield['Z_B_MAX'][i] - catfield['Z_B_MIN'][i])/2
                        if (sep <= 10):
                            ...
                            field_oneoverr[x][y][samplez<z_s0435] = field_oneoverr[x][y][samplez<z_s0435] + 0.1
                            field_zoverr[x][y][samplez<z_s0435] = field_zoverr[x][y][samplez<z_s0435] + (((z_s0435 * samplez[samplez<z_s0435]) - (samplez[samplez<z_s0435]**2)) / 10)
                            field_massoverr[x][y][samplez<z_s0435] = field_massoverr[x][y][samplez<z_s0435] + (samplemass[samplez<z_s0435] / 10)
                            field_mass2overr[x][y][samplez<z_s0435] = field_mass2overr[x][y][samplez<z_s0435] + ((samplemass[samplez<z_s0435]**2) / 10)
                            field_mass3overr[x][y][samplez<z_s0435] = field_mass3overr[x][y][samplez<z_s0435] + ((samplemass[samplez<z_s0435]**3) / 10)
                            field_zmassoverr[x][y][samplez<z_s0435] = field_zmassoverr[x][y][samplez<z_s0435] + (((z_s0435 * samplez[samplez<z_s0435]) - (samplez[samplez<z_s0435]**2)) * samplemass[samplez<z_s0435] / 10)
                            field_zmass2overr[x][y][samplez<z_s0435] = field_zmass2overr[x][y][samplez<z_s0435] + (((z_s0435 * samplez[samplez<z_s0435]) - (samplez[samplez<z_s0435]**2)) * (samplemass[samplez<z_s0435]**2) / 10)
                            ...
                            field_oneoverr[x][y][samplez<z_s0435] = field_oneoverr[x][y][samplez<z_s0435] + 1/sep
                            field_zoverr[x][y][samplez<z_s0435] = field_zoverr[x][y][samplez<z_s0435] + (((z_s0435 * samplez[samplez<z_s0435]) - (samplez[samplez<z_s0435]**2)) / sep)
                            field_massoverr[x][y][samplez<z_s0435] = field_massoverr[x][y][samplez<z_s0435] + (samplemass[samplez<z_s0435] / sep)
                            field_mass2overr[x][y][samplez<z_s0435] = field_mass2overr[x][y][samplez<z_s0435] + ((samplemass[samplez<z_s0435]**2) / sep)
                            field_mass3overr[x][y][samplez<z_s0435] = field_mass3overr[x][y][samplez<z_s0435] + ((samplemass[samplez<z_s0435]**3) / sep)
                            field_zmassoverr[x][y][samplez<z_s0435] = field_zmassoverr[x][y][samplez<z_s0435] + (((z_s0435 * samplez[samplez<z_s0435]) - (samplez[samplez<z_s0435]**2)) * samplemass[samplez<z_s0435] / sep)
                            field_zmass2overr[x][y][samplez<z_s0435] = field_zmass2overr[x][y][samplez<z_s0435] + (((z_s0435 * samplez[samplez<z_s0435]) - (samplez[samplez<z_s0435]**2)) * (samplemass[samplez<z_s0435]**2) / sep)
...
# write a file where the most probable value of z and Mstar has been used, and a file for each of the 100 or 1000 samples
        f = open('%s_q50_orig_i23.lst' % [x[0:len(listfields[0])-1] for x in listfields][count],'w')
        g = open('%s_q50_samp_i23.lst' % [x[0:len(listfields[0])-1] for x in listfields][count],'w')
        for i in range(cells_on_a_side):
            for j in range(cells_on_a_side):
        ...
        f = open('%s_q75_orig_i23.lst' % [x[0:len(listfields[0])-1] for x in listfields][count],'w')
        g = open('%s_q75_samp_i23.lst' % [x[0:len(listfields[0])-1] for x in listfields][count],'w')
        for i in range(cells_on_a_side):
            for j in range(cells_on_a_side):
