...
for i in range(len(list_cutout)):
    ...
    list_psf_ra[i] = coord[0]
    list_psf_dec[i] = coord[1]
    file.close()
    str = list_psf[i].split("-")
    filter = str[7]
    list_cutout_filter[i] = filter

np.savetxt("cutouts.cat",np.c_[list_cutout,list_cutout_filter,list_cutout_ra,list_cutout_dec])
np.savetxt("psf.cat",np.c_[list_psf,list_psf_filter,list_psf_ra,list_psf_dec],fmt = '%s %s %.18e %.18e')
